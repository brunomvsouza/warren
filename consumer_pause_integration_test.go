...
// TestConsumer_PauseThenForceReconnect_ClearsStalePause_integration proves the
// reconnect-during-pause contract end-to-end through the REAL reconnect path (not the
// unit deliverySubOverride): a Pause followed by a forced TCP reconnect re-subscribes
// the consumer and clears the stale pause, so Health reports Active again WITHOUT a
// Resume call, messages flow, and a subsequent Resume is a harmless no-op (T53).
func TestConsumer_PauseThenForceReconnect_ClearsStalePause_integration(t *testing.T) {
	defer goleak.VerifyNone(t)

	const total = 20
	const q = "test.pause.reconnect"
	...
	topo := &warren.Topology{Queues: []warren.Queue{{Name: q, Durable: true}}}
	require.NoError(t, topo.Declare(ctx, conn))
	require.NoError(t, topo.AttachTo(conn)) // re-declare on every reconnect
	...
	// — Pause: broker drops the subscription —————————————————————————————————
	require.NoError(t, consumer.Pause(ctx))
	require.Eventually(t, func() bool { return consumerCount() == 0 },
		5*time.Second, 50*time.Millisecond, "Pause must drop the broker-side subscription")
	h, err := consumer.Health(ctx)
	require.NoError(t, err)
	require.NotNil(t, h)
	require.True(t, h.Paused, "Health.Paused is true while paused")

	// — ForceReconnect WHILE paused: the reconnect barrier re-subscribes, and the
	// fresh subscription clears the stale pause — no Resume call ————————————————
	require.NoError(t, conn.ForceReconnect())

	require.Eventually(t, func() bool { return consumerCount() >= 1 },
		10*time.Second, 50*time.Millisecond, "reconnect must re-subscribe the consumer")
	require.Eventually(t, func() bool {
		hh, e := consumer.Health(ctx)
		return e == nil && hh != nil && !hh.Paused && hh.Active
	}, 5*time.Second, 50*time.Millisecond, "reconnect must clear the stale pause without a Resume")

	// Messages published now flow to the handler (the consumer is live again).
	pub, err := warren.PublisherFor[string](conn).Exchange("").RoutingKey(q).Build()
	require.NoError(t, err)
	for i := range total {
		body := "msg-" + strconv.Itoa(i)
	...
	require.Eventually(t, func() bool { return received.Load() == total },
		10*time.Second, 50*time.Millisecond, "all %d messages must be delivered after reconnect", total)

	// A Resume after the pause was already cleared by reconnect is a harmless no-op.
	require.NoError(t, consumer.Resume(ctx), "Resume on an already-cleared pause is a no-op")
	assert.Equal(t, int64(total), received.Load(), "Resume no-op must not change delivery")
