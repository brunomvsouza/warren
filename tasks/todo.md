...
- **Acceptance:**
  ...
  - [ ] **Lens-12 (DX-12):** the load-bearing `Message[M]` warning must live in the **call-site godoc**, not only in SPEC — add the **`UserID` 406-channel-close footgun** (§6.5 L1487-1513: a `UserID` that is not the authenticated connection user makes the broker close the channel) and the **`DeliveryMode` Persistent-is-the-zero-value-default** note (§6.5 L1450/L1554-1556) to the mandated field godoc, so a first-hour reader hovering the field sees the hazard without opening SPEC. dep XG-1.
...
- **Acceptance:**
  ...
  - [ ] **Lens-12 (DX-13):** route the caveat to the **call-site godoc** — add the **`ConfirmTimeout(0)`-discouraged** (§6.2 L972-978), **`PrefetchBytes()`-no-op-on-RabbitMQ** (§6.3 L1145), and **`ChannelQoS()`-per-channel-semantics** (§6.3 L1146) caveats, today SPEC prose / code comments only, to the godoc on the exported option/builder method, so they are visible where the user sets them. dep XG-1.
