...
### [ ] T166d — Campaign: SingleActiveConsumer failover on real node kill [P1] · M
- **Acceptance:**
  - [ ] Active consumer pinned to node A, hot standby on node B (each on its own warren connection over distinct addrs); a numbered sequence is published; node A is **killed** mid-sequence.
  - [ ] The broker promotes the standby; publish-order == handler-order across the real node death; the deduped accepted stream is exactly `0..N-1` (at-least-once handoff duplicate tolerated).
