...
- **Acceptance:**
  ...
  - [x] **`WithOnResubscribe(queue string)`** — recover-guarded inside the `notifyResubscribed` seam (`connection.go`). *(Sixth user callback; introduced one commit earlier in T34 and missed by T34c's original "five sites" count. Closed by the `origin/main..HEAD` range review (finding I-1): it runs inside the reconnect barrier on the supervisor goroutine, so an unrecovered panic crashed the process — but the replacement subscription is already installed by the time it fires, so it is log-and-continue like `WithOnReconnect`, NOT part of the deferred Lens-08 infra-hook work below. Test: `TestNotifyResubscribed_panickingCallback_recoversAndLogs_metricStillRecorded`.)*
  ...
  - [ ] **Lens-08 (CR-05):** the five sites above cover user *callbacks* but not the **infra-goroutine boundaries** nor `OnReturn`. Add a `recover` (a) in `reconnect.Loop.run` around the user `connect` fn (`internal/reconnect/loop.go:72`; today a panic crashes the whole process — `defer close(l.done)` runs but there is no recover) and (b) on the supervisor / `runBarrier` around the resubscribe **hook body itself** (`consumer.go` / `batch_consumer.go`; a panic in the hook's own work — e.g. `openDeliveryCh` — still kills the supervisor → reconnect silently disabled for that socket). NOTE: the *user* `WithOnResubscribe` callback inside that hook is already recover-guarded (review I-1, sixth acceptance box above) — it needs only log-and-continue, not socket-degrade, since the subscription is installed before it fires; what remains here is the infra hook body, which DOES need degrade-the-socket. Plus the missing `OnReturn` recover (CR-01, owned by T144). A panic in the infra paths must **degrade the socket** (`WithOnTopologyDegraded` + a metric), never crash the process or silently disable reconnect. *dep T143 (CG-5).*
...
- **Deferred (Lens-08 / CR-05, gated by T143):** the infra-goroutine boundaries (reconnect-loop `connect` fn, the resubscribe **hook body**) + the missing `OnReturn` recover are **not** done here. They require the **"degrade the socket" mechanism (a new metric)** and are explicitly **gate-first**: T143/CG-5 must first capture the current blast radius (process crash / silent reconnect-disable) before the fix lands. The plan sequences this in Phase 11 sub-phase (C) (`CG-5→T34c`); doing it now would violate the gate-first discipline. `OnReturn`'s recover is owned by T144, not T34c.
- **Follow-up (post-`origin/main..HEAD` range review):** Two findings fixed in `connection.go` (+ tests), separate from the deferred Lens-08 infra work above:
...
### [x] T166c — Campaign: quorum leader failover under sustained load (zero loss) [P1] · M
- **Acceptance:**
  - [x] A publisher streams confirmed messages to a **quorum** queue under load while the harness kills the leader node mid-stream; a consumer drains.
  - [x] Zero loss asserted as published − consumed deduped by `MessageID` (reuse the T45/TV-09 helpers), tolerating at-least-once duplicates; a non-vacuity floor (well above 1) proves the publisher survived the election.
- **Verify:** the test is green across a real Raft re-election; `goleak.VerifyNone`; the loss set is empty.
- **Files:** `cluster_quorum_failover_cluster_test.go`.
- **Deps:** T166b; reuses T45 loss accounting. **(T166-decomp / LT-05-cluster, P1)**
- **Done:** `cluster_quorum_failover_cluster_test.go` streams confirmed messages to a durable quorum queue (default-exchange routed, routing key == queue name) under a continuous publisher loop while a 4-concurrency consumer drains into a `MessageID`-deduped set; loss is the published−consumed set via the untagged `lossByMessageID` (the same TV-09 accounting `chaos_reconnect_integration_test.go` uses, self-tested with an injected drop in `chaos_reconnect_loss_test.go`). Two non-vacuity floors — a warmup batch confirmed BEFORE the kill and a second batch confirmed AFTER the new leader is elected — prove the publisher survived the re-election (a wedge-after-election would never reach the post floor), plus a recovery sentinel proves the consumer re-subscribed end-to-end. **Robustness:** rather than pinning the leader to a fixed node, the test dials the management node (rmq0) LAST so the leader always lands on a killable node, then discovers the actual leader via `QuorumLeader`/`NodeService` and kills THAT node — making the campaign order-independent (an earlier cluster test leaves rmq1 catching up, so the leader deterministically lands on rmq2 in-suite; the dynamic discovery handles whichever placement occurs). Added a reusable readiness gate to the toolkit, **`amqptest.WaitClusterReady`** (pure `parseRunningNodes`/`fetchRunningNodes` over `/api/nodes` in the untagged `cluster_control.go`, default-lane httptest unit-tested in `cluster_control_test.go`; cluster-tagged wrapper in `cluster.go`), so a campaign waits for all members back before declaring (member-count + majority safety). **Verified:** default `-race` suite + new amqptest unit tests green, `go vet`/`golangci-lint` (default + `--build-tags=cluster`) 0 issues, and the live cluster lane ran **4×** green in-suite (leader migrations observed rmq2→rmq0 and rmq2→rmq1, confirmed counts 74–210, elections 8–16 s) — always zero loss, always `goleak`-clean.

### [x] T166d — Campaign: SingleActiveConsumer failover on real node kill [P1] · M
- **Acceptance:**
  - [x] Active consumer pinned to node A, hot standby on node B (each on its own warren connection over distinct addrs); a numbered sequence is published; node A is **killed** mid-sequence.
  - [x] The broker promotes the standby; publish-order == handler-order across the real node death; the deduped accepted stream is exactly `0..N-1` (at-least-once handoff duplicate tolerated).
- **Verify:** the cluster-grade counterpart of `examples/ordered_consume` (which fakes failover by closing one connection) and `consumer_resubscribe` is green; `goleak`-clean.
- **Files:** `cluster_sac_failover_cluster_test.go`.
- **Deps:** T166b. **(T166-decomp / LT-05-cluster, P1)**
- **Done:** `cluster_sac_failover_cluster_test.go` declares a durable **quorum** queue with `SingleActiveConsumer` and subscribes two `Concurrency(1)`/`Prefetch(1)` consumers, each on its **own single-node connection** pinned to a distinct member — `connActive`→rmq1 (subscribes first → SAC active), `connStandby`→rmq2 (hot standby) — while `connMain`→rmq0 publishes, declares, and observes. Leader placement is pinned to the **surviving** rmq0 (declare-on-rmq0 + `queue_leader_locator=client-local`), isolating the variable under test (the active **consumer's** node death) from the queue-**leader** death T166c already covers; a precondition asserts `leader != rabbit@rmq1`. The active consumer handles the prefix `0..9` in order; its node is **`docker kill`-ed** mid-sequence; the broker promotes the standby, proven via an out-of-band readiness probe (seq=-1) before the suffix is published (no magic sleep). The shared ordered sink dedupes by sequence (tolerating the single in-flight message requeued to the standby at the handoff), records **which** consumer accepted each new seq, and asserts the deduped accepted stream is exactly `0..19` with `active=10`/`standby=10` — proving a **real** failover, not the active consumer quietly draining all N. **goleak gap found + fixed in-test:** the active consumer's node is killed behind a **still-listening Toxiproxy proxy**, so a reconnect dial to the dead node accepts the TCP connect but hangs the (non-context-aware) amqp091 handshake ~30 s, and `Close` cannot drain the supervisor blocked on it; resolved with a **bounded dialer** (`WithDialer` = `DefaultDial` with a 5 s connect+handshake deadline) — amqp091 clears the deadline on a successful handshake (`openComplete`), so healthy connections are unaffected while a dial to the dead node fails fast. **Verified live:** `make cluster-up && make test-cluster` green (all four `_cluster` campaigns: T166a/b/c/d); T166d ran **4×** in-suite (1× solo + 3× `-count=3`), every run `active=10 standby=10 accepted 0..19 in order (leader rabbit@rmq0 survived)`, always `goleak`-clean; default `-race` lane + `go vet`/`golangci-lint` (default + `--build-tags=cluster`) 0 issues.

### [x] T166i — Campaign: SAC failover with a MULTI-message in-flight set (closes LATER-87; follow-on to T166d) [P1] · M
- **Acceptance:**
  - [x] Active consumer runs `Prefetch(N>1)`; a gated handler acks the prefix in order then parks, so seq `holdAt..prefix-1` accumulate UNACKED — a multi-message in-flight set.
  - [x] The broker is polled (management API `messages_unacknowledged`) until `>= floor` BEFORE the kill, so the multi-message requeue is non-vacuous (an observed broker-side fact, not a magic sleep).
  - [x] After the node kill, the broker redelivers the whole requeued set to the `Prefetch(1)` standby **in order**; the deduped accepted stream is exactly `0..N-1` (`verifyContiguous`), at-least-once duplicates tolerated and counted.
- **Verify:** out-of-order violations empty across the multi-message requeue; `active==holdAt`, `standby==N-holdAt`; `goleak`-clean.
- **Files:** `cluster_sac_failover_multi_cluster_test.go`; `internal/amqptest/cluster_control.go` (+`_test.go`) `MessagesReady`/`MessagesUnacknowledged`; `cluster_sac_failover_cluster_test.go` `sacOrderTracker.duplicates()`.
- **Deps:** T166b, T166d. **(T166-decomp / LT-05-cluster / closes LATER-87, P1)**
- **Done:** `cluster_sac_failover_multi_cluster_test.go` runs the active consumer at `Concurrency(1)`/**`Prefetch(32)`** on rmq1 and the standby at `Concurrency(1)`/`Prefetch(1)` on rmq2 (the strict in-order verifier), with the same leader-survives-on-rmq0 isolation as T166d. The whole prefix `0..13` is published up front; a gated handler acks `0..3` in order then **parks on seq 4 before accepting it**, so seq `4..13` sit checked-out UNACKED behind the parked `Concurrency(1)` worker. The test polls the management API (`messages_unacknowledged`, newly exposed on `QuorumQueueState`) until `>= 6`, an **observed broker-side fact** that the in-flight set is multi-message (no magic sleep), then `docker kill`s rmq1. The broker requeues the whole `4..13` set; the standby (promoted, proven via the seq=-1 readiness probe) handles them **in order** then the suffix `14..23`. The held seqs are blocked before `tracker.accept`, so they reach the standby as fresh in-order NEW deliveries; `sacOrderTracker.duplicates()` makes any at-least-once redelivery (e.g. an ack lost to the SIGKILL) observable. **Verified live:** `make cluster-up && make test-cluster` green — all **5** `_cluster` campaigns pass (T166a/b/c/d + T166i), `goleak`-clean; T166i ran **4×** (1× solo + 3× `-count=3`), every run identical: **10 unacked held before the kill, active credited 4, standby credited 20, 0 duplicates, accepted 0..23 in order (leader rabbit@rmq0 survived)** — confirming RabbitMQ quorum+SAC preserves order across a multi-message requeue at promotion. Default `-race` lane (incl. the new `internal/amqptest` parse case) + `go vet`/`golangci-lint` (default + `--build-tags=cluster`) 0 issues.

### [x] T166e — Implement T66 per-connection `WithAddrs` shuffle + campaign: real rotation & no-`addr[0]` stampede [P1] · M
- **Acceptance:**
  - [x] *Code (TDD):* implement the per-connection **shuffle** of the `WithAddrs` list at `Dial` in `connection.go`/`options_connection.go` (today only the T33 round-robin cursor exists, `connection.go:468`); a deterministic unit test asserts N connections start from a distribution of addresses and reconnect rotates to a different address.
  - [x] Cluster campaign: N connections spread across the 3 nodes; a **full-cluster restart** shows reconnections spread across addresses (**no `addr[0]` stampede** — the DS-10/SRE-04/T66 assertion, unrunnable single-node).
  - [x] A real-node variant of `connection_failover_integration_test.go`'s dead-port trick.
- **Verify:** the shuffle unit test is deterministic and green; the cluster restart shows no `addr[0]` stampede; **this slice satisfies T66**; `goleak`-clean.
- **Files:** `connection.go`, `options_connection.go`, `connection_internal_test.go`, `cluster_failover_rotation_cluster_test.go`.
- **Deps:** T166b. **(T166-decomp / satisfies T66 / LT-01/05, P1)**
- **Done:** Each `managedConn` now walks its OWN permutation of the `WithAddrs` list instead of the shared in-order cursor: `shuffledAddrs` (deterministic Fisher–Yates over a per-socket seed, `math/rand/v2` PCG) builds the permutation lazily in `dialOrder()`; `nextDialAddr` advances the round-robin cursor over THAT order; `openPool` derives each socket's seed via `perConnSeed(base, role, idx)` (FNV-folded role XOR golden-ratio-mixed index), and `applyConnDefaults` seeds the process-level base from `rand.Uint64()` so every client process spreads differently (no cross-process stampede). The `WithAddrs` godoc and the two `connection_failover_integration_test.go` comments were updated to the shuffled-order contract (their assertions are order-independent and still pass, verified 3× live against the integration broker). Fulfils SPEC **R10-11**. Deterministic unit tests (`connection_internal_test.go`): `shuffledAddrs` is a stable permutation / copies / leaves <2 unchanged; distinct seeds spread the first address; `perConnSeed` is deterministic + distinct per (role,idx); `dialOrder` caches; **`TestPerConnSeeding_distributesInitialDialNoAddr0Stampede`** mirrors `openPool`'s seeding to prove the no-stampede distribution. **Cluster campaign** `cluster_failover_rotation_cluster_test.go`: `SpreadAndRestart` dials 6 sockets across the 3 nodes (shuffle seed pinned via the cluster-only `WithAddrShuffleSeedForTest` seam for a deterministic spread), asserts the initial pool spans ≥2 nodes, `ForceReconnect`s and asserts the genuinely-replaced pool (tracked by the broker's per-socket connection name via the new `amqptest.ConnNode`/`ConnectionNodes` + `parseConnectionNodes`/`fetchConnectionNodes`, default-lane unit-tested) re-spreads ≥2, then proves a rolling full-cluster restart leaves the pool reconnected + healthy; `RealNodeDeadFailover` stops a real node and proves Dial fails over to the survivor (the real-node analog of the dead-port trick). **Cross-task fix:** T166b/T166c relied on the pre-T66 "every socket dials addrs[0] first" trick to pin the quorum leader — the shuffle breaks that, so both now place the leader via the shuffle-immune single-address `declareQuorumLeaderOnNode` helper (matching the T166d/i pattern). **Verified live:** `make cluster-up` + the full cluster lane green — all **7** `_cluster` campaigns pass in order (T166a/b/c/d/e×2/i), `go test -race -tags=cluster ./` `ok` 122.5s, `make test-cluster` 0 failures, `goleak`-clean; the rotation campaign ran 2× confirming the pinned-seed spread; default `-race` lane + `go vet`/`golangci-lint` (default + `--build-tags=cluster`) 0 issues; integration failover tests 3× green. Environment restored (cluster down, integration broker healthy).

### [x] T166f — Campaigns: quorum confirm-latency under Raft + reconnect-storm across nodes [P2] · M
- **Acceptance:**
  - [x] Measure publisher-confirm latency on a quorum queue (3-node majority commit) under load; override `WithLatencyBuckets` so the 30 s `ConfirmTimeout` + R10-8 barrier tail is not clipped to `+Inf` (coordinate with the T71/T169 measurement theme); report p99/p999.
  - [x] Reconnect-storm: `ForceReconnect` across all nodes under load; assert recovery + zero loss + no `addr[0]` stampede (consumes T166e's shuffle).
- **Verify:** the latency histogram captures the tail (no `+Inf` clip); the storm test is zero-loss and `goleak`-clean.
- **Files:** `cluster_confirm_latency_cluster_test.go`, `cluster_reconnect_storm_cluster_test.go`.
- **Deps:** T166c, T166e. **(T166-decomp / LT-05-cluster, P2)**
- **Done:** `cluster_confirm_latency_cluster_test.go` streams 1800 confirmed publishes to a quorum queue across 6 concurrent publishers and reads back the built-in `publisher_publish_seconds` histogram — which records `time.Since(start)` AFTER `tracker.Wait` (`publisher.go`), so on a quorum queue it IS the 3-node Raft majority-commit latency. The campaign **overrides** the default buckets (the SPEC §9 `WithLatencyBuckets` override, threaded as the `buckets` argument of `NewPrometheusPublisherMetrics`) with a **second-valued** list whose top finite boundary is **35 s > the 30 s `ConfirmTimeout`** — *unit reality:* `RecordPublish` observes `d.Seconds()` and the series is named `publisher_publish_seconds`, so buckets are seconds even though SPEC §9 documents the default range in "ms"; reconciling that wording with the seconds observation is owned by T71/T169 (default-bucket fit), not this campaign. Asserts (a) every confirmed publish is one sample, (b) **no `+Inf` clip** — the largest finite bucket's cumulative count equals the total, (c) structural: the top finite bucket exceeds the `ConfirmTimeout` so a confirmed publish *cannot* clip by construction, (d) p50/p99/p999 (PromQL-style linear-interpolation `quantileFromBuckets`) are **finite** (a clipped tail reads `+Inf`). Live: `p50≈6.8ms p99≈15.7ms p999≈24.1ms`, tail captured. `cluster_reconnect_storm_cluster_test.go` pins the shuffle seed (`WithAddrShuffleSeedForTest` = `stormShuffleSeed`), spreads a 3+3 pool across the nodes, streams confirmed load + a draining consumer, then fires **3 `ForceReconnect` waves** with every node up; each wave waits for the pool to be fully **replaced** (`awaitReplacedPool`) and asserts the genuinely-new pool still spans **≥2 nodes** (no `addr[0]` stampede — consumes the T66/T166e shuffle), then proves recovery (`Health` green + clean pool + a recovery-sentinel publish→consume round-trip) and **zero loss** (`lossByMessageID(published, consumed)` empty, deduped by `MessageID` per TV-09, duplicates tolerated). **Verified live:** `make cluster-up && make test-cluster` green — all **9** `_cluster` tests pass interleaved with the node-killing campaigns (incl. T166f), `goleak`-clean; both new campaigns ran **4×** (1× solo + 3× `-count=3`), deterministic (storm: `confirmed==consumed-distinct`, every wave ≥2 nodes; latency: tail finite, no clip). Default `-race` lane + `go vet`/`go build`/`golangci-lint` (default + `--build-tags=cluster`) 0 issues. No SPEC/README change (the §7 harness amendment is T166g; the CI-lane + `make`-target README sync is T166h). Environment restored.

### [x] T166g — Campaigns: partition-under-load + rolling 3.13→4.x upgrade [P2] · M
- **Acceptance:**
  - [x] Partition-under-load: inject a partition under sustained load (via `docker network disconnect`, NOT Toxiproxy — Toxiproxy fronts only the AMQP client ports and cannot sever inter-node Erlang distribution, so it can never trigger `pause_minority`; see SPEC §7); assert `pause_minority` isolates the minority (running-node count drops to the majority), the majority surfaces **classifiable errors** (not silent stalls/OOM), and zero loss + recovery after heal.
  - [x] Rolling upgrade: mixed-version members, restart one node at a time under load, assert continuity (no loss, consumers resubscribe). Mixed-version is opt-in via `WARREN_RMQ2_IMAGE` and validated for **feature-flag-compatible** pairs (3.13.7 + 3.13.6); a fresh **4.x** node will not cluster with 3.13 (`incompatible_feature_flags`, confirmed live), so the genuine 3.13→4.x **major** in-place rolling upgrade is deferred to **LATER-88**.
  - [x] SPEC §7 amended (cluster harness + per-campaign topology labels).
- **Verify:** both campaigns run green on the cluster lane; `make lint` clean; SPEC §7 reflects the harness + topology labels.
- **Files:** `test/docker-compose.cluster.yml`, `internal/amqptest/cluster.go`, `internal/amqptest/cluster_control.go`, `cluster_partition_cluster_test.go`, `cluster_rolling_upgrade_cluster_test.go`, `SPEC.md` §7, `LATER.md` (LATER-88).
- **Deps:** T166b, T166e. **(T166-decomp / LT-05-cluster, P2)**
- **Done:** Two campaigns + the harness primitives behind them. **Harness (pure, fast-lane unit-tested):** `PartitionNode`/`HealPartition` (real broker partition via `docker network disconnect/connect`, with `--alias` restore on heal — Toxiproxy fronts only AMQP and cannot trigger pause_minority), `TryWaitRunningNodes` (non-fatal, exact-count, tolerant of the management-API hang during an undetected partition), `RunningNodes`, `RestartNode`, and `NodeVersions`/`parseNodeVersions` (rabbit-app version per node, for homogeneous-vs-mixed honesty). **Partition campaign** (`cluster_partition_cluster_test.go`): streams confirmed load to a quorum queue (leader pinned to a survivor), network-partitions a follower, asserts the minority is isolated (running drops to 2/3 via a tolerant poll — the per-quorum-queue stats read HANGS during the partition, so it is avoided), the publisher keeps confirming (no silent stall) with only classifiable sentinels, and zero loss after heal. A pause_minority node does not auto-resume after a full network cut (returns `running_partitioned_network`), so the heal reconnects the network AND restarts the node for a clean rejoin. **Rolling-upgrade campaign** (`cluster_rolling_upgrade_cluster_test.go`): streams confirmed load, restarts each node in turn (mgmt node last), asserts per-restart continuity + consumer resubscribe + zero loss; version-aware (logs homogeneous vs mixed). **Verified live** (homogeneous 3-node 3.13.7): partition ×2 (running=2/3 isolated→rejoined, confirmed≈18k, zero loss, ~95s/run) + rolling-upgrade (confirmed=7848, zero loss, 3 restarts); **mixed-version** rolling-upgrade on 3.13.7+3.13.6 (logged `MIXED-VERSION cluster in effect ([3.13.6 3.13.7])`, confirmed=6293, zero loss). 3.13.7+4.0.9 confirmed the feature-flag incompatibility live → LATER-88. Default `-race` fast lane + `golangci-lint` (default + `--build-tags=cluster`) + `go vet -tags=cluster` 0 issues; gofmt clean. No README change (the cluster-lane + make-target README sync is T166h's item; T166g adds no public surface).

### [x] T166h — Cluster CI lane wiring (+ homogeneous 3.13/4.x matrix) [P2] · M
- **Acceptance:**
  - [x] Wire the on-demand/nightly `cluster` lane into a scheduled workflow — brings up the compose cluster and runs `make test-cluster` on its stated cadence; **explicitly not a required per-PR gate** (D5). A standalone `cluster.yml` (schedule + `workflow_dispatch`, mirroring `bench.yml`), since T151's general nightly workflow has not yet landed (T151 consolidates it later).
  - [x] **Homogeneous 3.13 + 4.x version matrix** (`WARREN_RMQ_IMAGE`): the cluster campaigns run on BOTH a 3.13.7 and a 4.x cluster (the cluster-lane analogue of T151/TV-05's single-node integration matrix), proven to pass on 4.x (Khepri default, quorum changes) before the axis is wired.
  - [x] README synced (cluster lane + `make` targets + the `WARREN_RMQ_IMAGE`/`WARREN_RMQ2_IMAGE` knobs = external contract); the standing-env deferral remains **LATER-49** (confirm pointer, no new entry).
- **Verify:** the scheduled workflow runs the cluster lane on its stated cadence over both versions and is not a per-PR required check; both versions pass `make test-cluster` live; README reflects the lane + targets; `make lint` clean.
- **Files:** `.github/workflows/cluster.yml`, `README.md` (Makefile targets already exist from T166a).
- **Deps:** T166a–T166g (wires the lane that runs them); coordinates with T151; defers to LATER-49. **(T166-decomp / LT-05-cluster, P2)**
- **Done:** `.github/workflows/cluster.yml` (NEW) runs the lane on a nightly `schedule` + `workflow_dispatch` (mirroring `bench.yml`; **not** a per-PR gate) over a homogeneous **`WARREN_RMQ_IMAGE`** matrix — `3.13.7` + `4.0.9`, `fail-fast: false` so neither version masks the other — with per-version job-summary + `cluster-log-<label>` artifact; action pins match the repo's canonical SHAs. Two robustness fixes the 4.x validation surfaced shipped first (commit 52ea252): `make test-cluster` gained **`-count=1`** (the live cluster is external state outside Go's test-cache key — without it a re-run against a different image returns a vacuous cached green), and the rolling-upgrade recovery gate now **polls** `awaitQueueFullyOnline` instead of reading `QuorumLeader` once (a just-restarted node's quorum replica is a Member before its Raft follower reports Online — a lag sub-threshold on 3.13, observable on 4.x). SPEC §7 gained a **Version matrix** paragraph distinguishing `WARREN_RMQ_IMAGE` (all three nodes; the matrix axis) from `WARREN_RMQ2_IMAGE` (single member; the mixed-version rolling-upgrade knob). README Contributing block lists the cluster targets + knobs. **Verified live:** full `make test-cluster` green on a homogeneous **4.0.9** cluster (12 campaigns + unit suite, `-count=1`); `gofmt` clean; `golangci-lint run --build-tags=cluster .` 0 issues; `cluster.yml` YAML-validated (2 matrix legs, schedule + dispatch). Standing-env deferral remains LATER-49 (no new entry).

### Checkpoint — Phase 9.5
- [x] Harness (T166a/b): `make cluster-up` stands a 3-node quorum cluster + per-node Toxiproxy; the cluster env helper **fails, not skips**; the lane has a zero-run guard; the toolkit can kill a node and observe quorum-leader migration.
- [x] Campaigns (T166c–g): quorum leader failover / SAC node-kill / `WithAddrs` no-stampede / quorum confirm-latency / reconnect-storm / partition-under-load / rolling 3.13→4.x all run on the cluster lane and assert their outcomes; zero-loss reuses TV-09 dedup-by-`MessageID` unchanged.
- [x] Code (T166e): the T66 per-connection shuffle is implemented + unit-tested; the dead-port failover and `ordered_consume`/`consumer_resubscribe` patterns gain real-node-kill cluster counterparts; **the single-node `integration` lane is unchanged and stays the per-PR gate**.
- [x] Cadence: the `cluster` lane is **on-demand/nightly, not a required per-PR gate** (D5); the standing environment remains **LATER-49**.
- [x] `go build ./...` + `make lint` clean; `go test -race ./...` green (cluster tests excluded without the tag); SPEC §7 + README synced.
- [x] Phase-24 **T166** entry points here (decomposed → T166a–h); references to "T166" in T66/T165/T164-LG-2/LATER-49 resolve to the T166a–h family; tally updated (no duplicate IDs).
...
- [ ] SPEC §10 "Rev 23" note records the Lens-13 pass; five findings extend their owning task in place (T44b/T45/T66/T71/T151) + five groups confirmed (T45/TV-09, T44b/PC-04, T63/R10-8, T59/R10-3, T65), not re-filed; exactly **one** new `LATER.md` entry (LATER-49); T164–T169 contiguous, no duplicate IDs; tally **185 tasks / 25 phases** (Phase 9.5 decomposes T166 into T166a–T166h and pulls it forward; the Phase-24 T166 entry is now a pointer, not a counted leaf).
