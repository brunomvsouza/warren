...
- **Acceptance:**
  - [ ] A publisher streams confirmed messages to a **quorum** queue under load while the harness kills the leader node mid-stream; a consumer drains.
  - [ ] Zero loss asserted as published − consumed deduped by `MessageID` (reuse the T45/TV-09 helpers), tolerating at-least-once duplicates; a non-vacuity floor (well above 1) proves the publisher survived the election.
