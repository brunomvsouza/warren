...
### [ ] T166e — Implement T66 per-connection `WithAddrs` shuffle + campaign: real rotation & no-`addr[0]` stampede [P1] · M
- **Acceptance:**
  - [ ] *Code (TDD):* implement the per-connection **shuffle** of the `WithAddrs` list at `Dial` in `connection.go`/`options_connection.go` (today only the T33 round-robin cursor exists, `connection.go:468`); a deterministic unit test asserts N connections start from a distribution of addresses and reconnect rotates to a different address.
  - [ ] Cluster campaign: N connections spread across the 3 nodes; a **full-cluster restart** shows reconnections spread across addresses (**no `addr[0]` stampede** — the DS-10/SRE-04/T66 assertion, unrunnable single-node).
  - [ ] A real-node variant of `connection_failover_integration_test.go`'s dead-port trick.
