...
- [ ] Harness (T166a/b): `make cluster-up` stands a 3-node quorum cluster + per-node Toxiproxy; the cluster env helper **fails, not skips**; the lane has a zero-run guard; the toolkit can kill a node and observe quorum-leader migration.
- [ ] Campaigns (T166c–g): quorum leader failover / SAC node-kill / `WithAddrs` no-stampede / quorum confirm-latency / reconnect-storm / partition-under-load / rolling 3.13→4.x all run on the cluster lane and assert their outcomes; zero-loss reuses TV-09 dedup-by-`MessageID` unchanged.
- [ ] Code (T166e): the T66 per-connection shuffle is implemented + unit-tested; the dead-port failover and `ordered_consume`/`consumer_resubscribe` patterns gain real-node-kill cluster counterparts; **the single-node `integration` lane is unchanged and stays the per-PR gate**.
- [ ] Cadence: the `cluster` lane is **on-demand/nightly, not a required per-PR gate** (D5); the standing environment remains **LATER-49**.
- [ ] `go build ./...` + `make lint` clean; `go test -race ./...` green (cluster tests excluded without the tag); SPEC §7 + README synced.
- [ ] Phase-24 **T166** entry points here (decomposed → T166a–h); references to "T166" in T66/T165/T164-LG-2/LATER-49 resolve to the T166a–h family; tally updated (no duplicate IDs).
