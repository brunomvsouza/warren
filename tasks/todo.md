...
### [ ] T166g — Campaigns: partition-under-load + rolling 3.13→4.x upgrade [P2] · M
- **Acceptance:**
  - [ ] Partition-under-load: inject a partition via Toxiproxy under sustained load; assert `pause_minority` isolates the minority, the majority surfaces **classifiable errors** (not silent stalls/OOM), and zero loss + recovery after heal.
