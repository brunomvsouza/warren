...
### [ ] T166h — Cluster CI lane wiring (+ homogeneous 3.13/4.x matrix) [P2] · M
- **Acceptance:**
  ...
  - [ ] **Homogeneous 3.13 + 4.x version matrix** (`WARREN_RMQ_IMAGE`): the cluster campaigns run on BOTH a 3.13.7 and a 4.x cluster (the cluster-lane analogue of T151/TV-05's single-node integration matrix), proven to pass on 4.x (Khepri default, quorum changes) before the axis is wired.
  - [ ] README synced (cluster lane + `make` targets + the `WARREN_RMQ_IMAGE`/`WARREN_RMQ2_IMAGE` knobs = external contract); the standing-env deferral remains **LATER-49** (confirm pointer, no new entry).
