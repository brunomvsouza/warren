...
### [ ] T166f — Campaigns: quorum confirm-latency under Raft + reconnect-storm across nodes [P2] · M
- **Acceptance:**
  - [ ] Measure publisher-confirm latency on a quorum queue (3-node majority commit) under load; override `WithLatencyBuckets` so the 30 s `ConfirmTimeout` + R10-8 barrier tail is not clipped to `+Inf` (coordinate with the T71/T169 measurement theme); report p99/p999.
  - [ ] Reconnect-storm: `ForceReconnect` across all nodes under load; assert recovery + zero loss + no `addr[0]` stampede (consumes T166e's shuffle).
