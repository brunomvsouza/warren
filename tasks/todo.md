...
- **Acceptance:**
  - [ ] Wire the on-demand/nightly `cluster` lane into the scheduled workflow (**extends T151**) — brings up the compose cluster and runs `make test-cluster` on its stated cadence; **explicitly not a required per-PR gate** (D5).
  - [ ] README synced (cluster lane + new `make` targets = external contract); the standing-env deferral remains **LATER-49** (confirm pointer, no new entry).
- **Verify:** the scheduled workflow runs the cluster lane on its stated cadence and is not a per-PR required check; README reflects the lane + targets; `make lint` clean.
- **Files:** `.github/workflows/*.yml`, `Makefile`, `README.md`.
- **Deps:** T166a–T166g (wires the lane that runs them); extends T151; defers to LATER-49. **(T166-decomp / LT-05-cluster, P2)**
