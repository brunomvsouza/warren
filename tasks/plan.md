...
- **T166i** `cluster_sac_failover_multi_cluster_test.go` (+ `internal/amqptest/cluster_control.go` unacked counts): **campaign — SAC failover with a MULTI-message in-flight set (closes LATER-87; follow-on to T166d).** T166d's active consumer runs `Prefetch(1)`, so at most one in-flight message is requeued at the handoff. This sibling runs the active consumer at **`Prefetch(N>1)`**: a gated handler acks the prefix in order then parks, leaving the rest of the published prefix unacked; the test polls the management API until `messages_unacknowledged >= floor` (an observed broker-side fact — non-vacuous, no magic sleep), kills the active node, and asserts the broker redelivers the whole requeued set to the `Prefetch(1)` standby **in order** (deduped accepted stream still exactly `0..N-1`, `verifyContiguous`). Adds `MessagesReady`/`MessagesUnacknowledged` to `QuorumQueueState` and a `duplicates()` counter on the shared `sacOrderTracker` so the requeue path's coverage is observable. **Deps:** T166b, T166d. **(T166-decomp / LT-05-cluster / closes LATER-87, P1)**
...
- **T166h** `.github/workflows/*.yml` + `Makefile` + `README.md`: **cluster CI lane wiring.** Wire the on-demand/nightly `cluster` lane into a scheduled workflow — runs `make cluster-up`/`test-cluster`/`cluster-down` against the compose cluster on its stated cadence across a **homogeneous RabbitMQ 3.13 + 4.x version matrix** (`WARREN_RMQ_IMAGE`), so the cluster campaigns are proven on both the LTS estates run and the 4.x quorum/Khepri behaviour they assert; **explicitly not a required per-PR gate** (D5). *T166h creates a standalone `cluster.yml` (schedule + `workflow_dispatch`, mirroring `bench.yml`) because T151's general nightly workflow has not yet landed — when **T151** ships it consolidates/harmonises this lane; the 3.13/4.x cluster matrix here is the cluster-lane analogue of T151/TV-05's single-node integration matrix.* Sync README (cluster lane + `make` targets + the `WARREN_RMQ_IMAGE`/`WARREN_RMQ2_IMAGE` knobs — external contract). Confirm the standing-env deferral remains **LATER-49** (no new entry). **Deps:** T166a–T166g (wires the lane that runs them); coordinates with T151; defers to LATER-49. **(T166-decomp / LT-05-cluster, P2)**

**Checkpoint — Phase 9.5:**
- [x] Harness (T166a/b): `make cluster-up` stands a 3-node quorum cluster + per-node Toxiproxy; the cluster env helper **fails, not skips** when the cluster vars are unset; the `cluster` lane has a zero-run guard; the control toolkit can kill a node and observe quorum-leader migration via the management API.
- [x] Campaigns (T166c–g): quorum leader failover / SAC node-kill failover / `WithAddrs` no-stampede / quorum confirm-latency / reconnect-storm / partition-under-load / rolling 3.13→4.x all run on the cluster lane and assert their outcomes; zero-loss reuses the TV-09 dedup-by-`MessageID` method unchanged.
- [x] Code (T166e): the T66 per-connection shuffle is implemented + unit-tested; the dead-port failover test and the `ordered_consume`/`consumer_resubscribe` patterns gain real-node-kill cluster counterparts; **the single-node `integration` lane is unchanged and stays the per-PR gate.**
- [x] Cadence: the `cluster` lane is **on-demand/nightly, not a required per-PR gate** (D5); the standing dedicated environment remains **LATER-49**.
- [x] `go build ./...` + `make lint` clean; `go test -race ./...` green (cluster tests excluded without the tag); SPEC §7 + README synced (new make targets + lane = external contract).
- [x] Phase-24 **T166** entry points here (decomposed → T166a–h, pulled forward); **T66/T165/T164-LG-2/LATER-49** references to "T166" resolve to the T166a–h family; tallies updated (no duplicate IDs).
