...
### LATER-54 — No broker-side assertion that quorum+DLX declares `x-dead-letter-strategy=at-least-once` / `x-delivery-limit`

**Context:** `topology_integration_test.go` — the T15 acceptance says "assert via `rabbitmqctl` that the source
queue carries the DLX args / `x-delivery-limit`". The current integration tests (`TestTopology_Declare_dlxExpansion_integration`,
`TestTopology_Declare_quorumWithDeliveryLimit_integration`) only assert `Declare` returns `nil`; they never read the
queue arguments back from the broker. The new `x-dead-letter-strategy=at-least-once` injection (decision 52) is
therefore proven only against the in-memory `expand()` map (unit + snapshot tests) and the deadletter example's
end-to-end flow, never by reading the arg back from RabbitMQ. `QueueDeclarePassive` (the precedent in
`examples/topology`) returns existence only, not arguments — so broker-side arg inspection needs the management HTTP
API.

**Impact:** Minor/Important-for-completeness. The arg name/value are SPEC- and RabbitMQ-doc-derived and the
deadletter smoke-run was manually confirmed via `rabbitmqctl list_queues name type arguments` this session (the
quorum source carried `x-dead-letter-strategy=at-least-once`, `x-delivery-limit=3`, and the full DLX arg set). The
gap is automated evidence: a future change that drops or mis-types the strategy would pass unit tests and the smoke
flow (which would still dead-letter) but not be caught by a value assertion.

**Evidence:** `/agent-skills:ship` parallel review of branch `worktree-phase-3-validation`, 2026-05-29
(test-engineer Important gap #1 + code-reviewer suggestion).

**Suggested solution:** Add an `integration`-tagged `TestTopology_Declare_quorumDLXStrategy_integration` that declares
a quorum source queue with a `DeadLetter` entry + `DeliveryLimit(3)`, then queries the RabbitMQ management API
(`GET /api/queues/%2F/<name>`, derived host/creds from `AMQP_TEST_URL`) and asserts
`arguments["x-dead-letter-strategy"] == "at-least-once"`, `arguments["x-dead-letter-exchange"]` set, and
`arguments["x-delivery-limit"] == 3`. Factor a small management-API helper so other integration tests can reuse it.

**Prerequisites:** none functionally, but the management-API helper is shared infrastructure — coordinate with T37
(testcontainers helper) so the management port/creds discovery lives in one place.

---

### LATER-55 — Quorum declared via `Args["x-queue-type"]="quorum"` (not the `Type` field) silently misses the at-least-once injection

**Context:** `topology.go` `expand()` — the `x-dead-letter-strategy=at-least-once` injection is gated on the
`q.Type == QueueTypeQuorum` struct field. A caller who makes a quorum queue by setting
`Args["x-queue-type"] = "quorum"` directly (instead of `Type: QueueTypeQuorum`) gets a valid quorum queue at the
broker but no strategy injection, and `validate()` does not reject or normalize that path either.

**Impact:** Low. The `Type` field is the documented, idiomatic path and the one every test and example uses; the raw
`Args` route is an undocumented escape hatch. But a user on that route silently loses the at-least-once
dead-lettering guarantee — a quiet correctness footgun that contradicts the "at-least-once is the contract"
invariant.

**Evidence:** `/agent-skills:ship` parallel review of branch `worktree-phase-3-validation`, 2026-05-29
(code-reviewer suggestion).

**Suggested solution:** Pick one: (a) make the injection guard also treat `Args["x-queue-type"] == "quorum"` as
quorum, or (b) have `validate()` reject a raw `x-queue-type` arg with a message directing the caller to the `Type`
field (consistent with the library's "surface real primitives via typed fields" stance). Option (b) is cleaner — it
keeps a single source of truth for queue type. Add a unit test for the chosen behaviour.

**Prerequisites:** none (self-contained in `topology.go` + `topology_test.go`).

---

### LATER-56 — Add `FuzzTopologyExpand` to lock the no-panic guarantee of `validate()`+`expand()` against crafted Args

**Context:** `topology.go` — `validate()`/`expand()` accept caller-supplied `map[string]any` args and were confirmed
panic-safe under an adversarial probe (non-string `x-delayed-type`, nil Args, deeply nested maps for `copyArgs`
recursion, DeadLetter on an undeclared source). That guarantee is currently held only by hand-written cases, while
the project already treats fuzzing as part of the contract (`FuzzRedactURI`, `FuzzXDeathParser`).

**Impact:** Low (defense-in-depth). No current panic path exists; the risk is a future edit to the arg-injection or
validation logic reintroducing a nil-map write or an unchecked type assertion that the fixed unit cases miss.

**Evidence:** `/agent-skills:ship` parallel review of branch `worktree-phase-3-validation`, 2026-05-29
(security-auditor optional recommendation).

**Suggested solution:** Add a `FuzzTopologyExpand` that builds a `Topology` with fuzz-derived queue names, types, and
random `map[string]any` arg shapes, runs `validate()` then (if it passes) `expand()`, and asserts neither panics.
Seed it with the adversarial cases from the audit.

**Prerequisites:** none (self-contained fuzz target alongside the existing fuzz suite).
